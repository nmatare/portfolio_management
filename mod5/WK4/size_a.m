...
N = 10;                 % number of size-sorted portfolios: 3, 5, or 10
...
x = load('size_1927_2016.txt');     % load returns on size-sorted portfolios
years = x(:,1);
...
means = mean(r)
stds = std(r);
dif = r(:,1) - r(:,end);    % first minus last portfolio returns
...
%disp('Average portfolio returns (Small to Big)');
%disp(means)
%disp('Small-Big average return:')
%disp(mdif)
%disp('t-statistic')
%disp(tdif)
%fprintf(['\n']) % insert an empty line
...
if ismember(1,pick_figures)    
    ...
    bar(means)
    set(gca,'FontSize',10)
    set(gca,'XLabel',text(0,0,'S i z e   P o r t f o l i o  (S m a l l   t o   B i g)'))
    set(gca,'YLabel',text(0,0,'A v e r a g e   A n n u a l   R e t u r n  ( % )'))
    set(gca,'XLim',[0 N+1])
    figname = ['size_a_means_' int2str(period(1)) '_' int2str(period(end))];
...
if ismember(2,pick_figures)    
    ...
    set(gca,'XLim',[0 N+1])
    figname = ['size_a_stds_' int2str(period(1)) '_' int2str(period(end))];
    set(gcf,'PaperPosition',[0.25,2.5,8,7.5])
    eval(['print -deps2 ' figname]); 
end
...
if ismember(3,pick_figures)    
    ...
    set(gca,'XLim',[0 N+1])
    figname = ['size_a_cm_betas_' int2str(period(1)) '_' int2str(period(end))];
    set(gcf,'PaperPosition',[0.25,2.5,8,7.5])
    eval(['print -deps2 ' figname]); 
end
...
if ismember(4,pick_figures)    
    ...
    set(gca,'XLim',[0 N+1])
    figname = ['size_a_cm_alphas_' int2str(period(1)) '_' int2str(period(end))];
    set(gcf,'PaperPosition',[0.25,2.5,8,7.5])
    eval(['print -deps2 ' figname]); 
end

if ismember(5,pick_figures)    
    % Plot moving averages of the S-B difference
    ...
    set(gca,'YLabel',text(0,0,'S i z e   P r e m i u m   ( %  p e r  y e a r )'))
    set(gca,'XLim',[9 T])
    %set(gca,'YLim',[0 ytop])
    v = [4:10:T];
    set(gca,'XTick',v);
    set(gca,'XTickLabel',period(v));
    figname = ['size_a_movavg_' int2str(period(1)) '_' int2str(period(end))];
    set(gcf,'PaperPosition',[0.25,2.5,8,7.5])
    eval(['print -deps2 ' figname]); 
end