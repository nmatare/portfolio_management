...
Our goal is to construct a portfolio with a standard deviation equal to 1.89%. Thus, we find the optimal weights such that variance of our goal portfolio is satisfied.

$\sigma_{P}$ = Std($R_{P}$) =  w(0.035) + 1-w(0) = 0.0189

w = 0.54

Instead of trying to time the market, we construct a passive portfolio with constant weights:

$E_{P}$ = $\mathbb{E}$($R_{P}$) = 0.70(0.015) + 0.30(0.01) = 1.35%

$\sigma_{P}$ = Std($R_{P}$) = 0.50(0.035) + 0.50(0)

We hold a 70/30 stock/cash mix, thus we expect to yield a 1.35% return while maintaining 
...
Natuarlly, the greater uncertainty increased the probability of underperformance. However, the effect of $\mu$ on the cost of shortfall insurance is zero. That is, the value of the option does not depend on $\mu$, so there is no change when T is either 10 or 20.
